...
public class GenerarReportes {
    ...
    public void credencialUsuario(String user) {
        try {
            ...
            try {
                InputStream f = new FileInputStream(new File(".").getCanonicalPath() + File.separator + "src" + File.separator + "proyector" + File.separator + "reportes" + File.separator + "grafeteS.jasper");
    ...
    public void getBarcodeProy() {
        try {
            Conexion conn = new Conexion();
            JasperReport report = null;
            try {
                String ruta = new java.io.File(".").getCanonicalPath() + File.separator + "src" + File.separator + "proyector" + File.separator + "reportes" + File.separator + "cdbr.jasper";
            ...
            genVistaR(j, "Códigos de barra Videoproyectores");
        } catch (JRException | SQLException e) {
            System.out.println("Error al generar los codigos de barra: " + e);
        }
    }

    public void getGafeteProy() {
        try {
            Conexion conn = new Conexion();
            JasperReport report = null;
            try {
                String ruta = new java.io.File(".").getCanonicalPath() + File.separator + "src" + File.separator + "proyector" + File.separator + "reportes" + File.separator + "proyGaf.jasper";
                InputStream f = new FileInputStream(ruta);
                System.out.println("Current dir Report:" + ruta);
                report = (JasperReport) JRLoader.loadObject(f);
            } catch (JRException | IOException e) {
                System.out.println("Error cargando plantilla del reporte " + e);
            }
            JasperPrint j = JasperFillManager.fillReport(report, null, conn.getConexion());
            genVistaR(j, "Gafetes de Videoproyectores");
        } catch (JRException | SQLException e) {
            System.out.println("Error al generar gafetes de Proyectores: " + e);
        }
    }
    ...
    public void getRRTods(String[] date, int opc, int dateOPC) {
        try {
            ...
            genVistaR(j, nomReporte);
        } catch (JRException | SQLException e) {
            System.out.println("Error al generar reporte TODO: " + e);
        }
    }

    public void getSolicitudMant(String[] datos) {
        try {
            Conexion conn = new Conexion();
            JasperReport report = null;
            String ruta = "";
            ...
            JasperPrint j = JasperFillManager.fillReport(report, parameter, conn.getConexion());
            genVistaR(j, "Reporte de Videoproyector");
    ...
    public void genVistaR(JasperPrint j, String encabezado) {
        JasperViewer jv = new JasperViewer(j, false);
        jv.setTitle(encabezado);
        jv.setVisible(true);
